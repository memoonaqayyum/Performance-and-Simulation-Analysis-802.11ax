...
def filtered_dict_product(d, filter_func):
    keys = d.keys()
    for element in product(*d.values()):
        candidate = dict(zip(keys, element))
...
def get_rate_and_coding_for_modulation_improved(mcs):
  
    if not isinstance(mcs, int) or not (0 <= mcs <= 11):
        raise ValueError("MCS must be an integer between 0 and 11 inclusive.")
...
def get_model_throughput_improved(validation_params, metrics=None, compute_func=None):
    """
    Calculate the model throughput based on the given validation parameters and metrics.
    
    Args:
    validation_params (dict): A dictionary of parameters to validate.
    metrics (list): A list of metrics to be used. Defaults to ['dl', 'ul', 'hol'].
    compute_func (function): A function to compute the model throughput. If None, a default function is used.

    Returns:
    xarray.DataArray: An array containing the model throughput for each combination of parameters.
    """
    if metrics is None:
        metrics = ['dl', 'ul', 'hol']
    if compute_func is None:
        compute_func = compute_model_throughput  # Assuming compute_model_throughput is defined elsewhere

    params_with_metrics = copy.deepcopy(validation_params)
    params_with_metrics['metrics'] = metrics
    model_output = xr.DataArray(np.zeros([len(i) for i in params_with_metrics.values()]),
                                list(zip(params_with_metrics.keys(), params_with_metrics.values())))

    with Pool() as pool:
        param_list = list(dict_product(validation_params))
        for param_comb, result in zip(param_list, pool.imap(compute_func, tqdm(param_list, total=len(param_list), desc="Running model", unit="Parameter combination"))):
            model_output.loc[param_comb] = result
